A quick MongoDB StatefullSet work for POW of on local storaged cluster node lock for certain STS resource to certain worker node. 
...
spawns the pod at that node. When the Pod is ready, volume is being mounted. So, the STS locked based on its PV!
